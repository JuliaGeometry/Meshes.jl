@testset "Sampling" begin
  @testset "UniformSampling" begin
    ...
    μ = mean(fromorigin.([centroid(s, i) for i in 1:nelements(s)]))
  ...
  @testset "BallSampling" begin
    ...
    n = nelements(s)
    x = fromorigin(centroid(s, 1))
    y = fromorigin(centroid(s, 17))
    @test n < 100
  ...
  @testset "HomogeneousSampling" begin
    ...
    @test all(zero(ℳ) ≤ coords[1] ≤ oneunit(ℳ) for coords in fromorigin.(ps))
    @test all(coords[2] == zero(ℳ) for coords in fromorigin.(ps))

    s = Segment(point(0, 0), point(0, 1))
    ps = sample(s, HomogeneousSampling(100))
    @test first(ps) isa Point{2}
    @test all(coords[1] == zero(ℳ) for coords in fromorigin.(ps))
    @test all(zero(ℳ) ≤ coords[2] ≤ oneunit(ℳ) for coords in fromorigin.(ps))

    s = Segment(point(0, 0), point(1, 1))
    ps = sample(s, HomogeneousSampling(100))
    @test first(ps) isa Point{2}
    @test all(zero(ℳ) ≤ coords[1] == coords[2] ≤ oneunit(ℳ) for coords in fromorigin.(ps))
