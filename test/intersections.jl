@testset "Intersections" begin
  ...
  @testset "Segments" begin
    ...
    s1 = Segment(
      P2(0.181429364026879, 0.546811355144474), P2(0.38282226144778, 0.107781953228536)
    )
    s2 = Segment(
      P2(0.412498700935005, 0.212081819871479), P2(0.395936725690311, 0.252041094122474)
    )
    @test s1 ∩ s2 === nothing
    @test s2 ∩ s1 === nothing
    ...
    @test type(intersection(s₁, s₂)) == CrossingSegments
    @test type(intersection(s₂, s₁)) == CrossingSegments

    @test s₁ ∩ s₃ ≈ s₃ ∩ s₁ ≈ P2(0, 0) # CASE 2: MidTouchingSegments (s₁(0))
    @test type(intersection(s₁, s₃)) == MidTouchingSegments
    @test type(intersection(s₃, s₁)) == MidTouchingSegments
    ...
    @test type(intersection(s₁, s₄)) == CornerTouchingSegments
    @test type(intersection(s₄, s₁)) == CornerTouchingSegments
    ...
    @test type(intersection(s₉, s₉)) == OverlappingSegments
    ...
    @test type(intersection(s₉, s₁₇)) == NoIntersection
    @test type(intersection(s₁₇, s₉)) == NoIntersection
    ...
    @test type(intersection(s1, s2)) == CrossingSegments
    @test s1 ∩ s2 ≈ P3(0.5, 0.0, 0.0)
    @test type(intersection(s1, s3)) == OverlappingSegments
    @test s1 ∩ s3 == Segment(P3(0.5, 0.0, 0.0), P3(1.0, 0.0, 0.0))
    @test type(intersection(s1, s4)) == MidTouchingSegments
    @test s1 ∩ s4 ≈ P3(0.0, 0.0, 0.0)
    @test type(intersection(s1, s5)) == MidTouchingSegments
    @test s1 ∩ s5 ≈ P3(0.0, 0.0, 0.0)
    @test type(intersection(s1, s6)) == CornerTouchingSegments
    @test s1 ∩ s6 ≈ P3(0.0, 0.0, 0.0)
    @test type(intersection(s1, s7)) == NoIntersection
    @test isnothing(s1 ∩ s7)
    @test type(intersection(s1, s8)) == NoIntersection
    @test isnothing(s1 ∩ s8)
    @test type(intersection(s1, s9)) == NoIntersection
    @test isnothing(s1 ∩ s9)
    @test type(intersection(s1, s10)) == NoIntersection
    @test isnothing(s1 ∩ s10)
    @test type(intersection(s1, s11)) == NoIntersection
    @test isnothing(s1 ∩ s11)
    @test type(intersection(s1, s12)) == CornerTouchingSegments
  ...
  @testset "Rays" begin
    ...
    r₇ = Ray(P2(4, 0), V2(0, 1))
    @test type(intersection(r₁, r₂)) == CrossingRays #CASE 1
    @test r₁ ∩ r₂ ≈ P2(1.25, 0.125)
    @test r₁ ∩ r₇ ≈ P2(4, 1.5)
    @test type(intersection(r₁, r₃)) == MidTouchingRays #CASE 2
    @test r₁ ∩ r₃ ≈ origin(r₁) #origin of first ray
    @test r₅ ∩ r₇ ≈ origin(r₅)
    @test type(intersection(r₃, r₁)) == MidTouchingRays
    @test r₃ ∩ r₁ ≈ origin(r₁) #origin of second ray
    @test r₇ ∩ r₅ ≈ origin(r₅)
    @test type(intersection(r₂, r₄)) == CornerTouchingRays #CASE 3
    @test r₂ ∩ r₄ ≈ origin(r₂) ≈ origin(r₄)
    @test type(intersection(r₅, r₁)) == OverlappingAgreeingRays #CASE 4
    @test r₅ ∩ r₁ == r₅ #first ray
    @test type(intersection(r₁, r₅)) == OverlappingAgreeingRays #CASE 4
    @test r₁ ∩ r₅ == r₅ #second ray
    @test type(intersection(r₁, r₆)) == OverlappingOpposingRays #CASE 5
    @test r₁ ∩ r₆ == Segment(origin(r₁), origin(r₆))
    @test type(intersection(r₁, r₄)) == NoIntersection #CASE 6
  ...
  @testset "RaySegment" begin
    ...
    @test type(intersection(r₁, s₁)) == CrossingRaySegment #CASE 1
    @test r₁ ∩ s₁ ≈ s₁ ∩ r₁ ≈ P2(1.25, 0.125)
    @test type(intersection(r₁, s₂)) == NoIntersection # CASE 5
    @test r₁ ∩ s₂ === s₂ ∩ r₁ === nothing
    @test type(intersection(r₁, s₃)) == NoIntersection # CASE 5
    @test r₁ ∩ s₃ === s₃ ∩ r₁ === nothing
    @test type(intersection(r₁, s₄)) == MidTouchingRaySegment # CASE 2
    @test r₁ ∩ s₄ ≈ s₄ ∩ r₁ ≈ origin(r₁)
    @test type(intersection(r₁, s₅)) == MidTouchingRaySegment # CASE 2
    @test r₁ ∩ s₅ ≈ s₅ ∩ r₁ ≈ P2(1.5, 0.25)
    @test type(intersection(r₁, s₆)) == CornerTouchingRaySegment # CASE 3
    @test r₁ ∩ s₆ ≈ s₆ ∩ r₁ ≈ origin(r₁)
    @test type(intersection(r₁, s₇)) == CornerTouchingRaySegment # CASE 3
    @test r₁ ∩ s₇ ≈ s₇ ∩ r₁ ≈ origin(r₁)
    ...
    @test type(intersection(r₂, s₈)) == OverlappingRaySegment # CASE 4
    @test r₂ ∩ s₈ === s₈ ∩ r₂ === s₈
    @test type(intersection(r₂, s₉)) == OverlappingRaySegment # CASE 4
  ...
  @testset "RayLine" begin
    ...
    @test type(intersection(l₁, r₁)) === CrossingRayLine

    @test l₁ ∩ r₂ == r₂ ∩ l₁ == origin(r₂) # CASE 2
    @test type(intersection(l₁, r₂)) === TouchingRayLine

    @test l₁ ∩ r₃ == r₃ ∩ l₁ == r₃ # CASE 3
    @test type(intersection(l₁, r₃)) === OverlappingRayLine

    @test l₁ ∩ r₄ == r₄ ∩ l₁ === nothing # CASE 4 parallel
    @test type(intersection(l₁, r₄)) === NoIntersection
  ...
  @testset "LineSegment" begin
    ...
    @test type(intersection(l₁, s₁)) == CrossingLineSegment #CASE 1
    @test l₁ ∩ s₁ ≈ s₁ ∩ l₁ ≈ P2(1.25, 0.125)
    @test type(intersection(l₁, s₂)) == NoIntersection # CASE 4
    @test l₁ ∩ s₂ === s₂ ∩ l₁ === nothing
    @test type(intersection(l₁, s₃)) == NoIntersection # CASE 4
    @test l₁ ∩ s₃ === s₃ ∩ l₁ === nothing
    @test type(intersection(l₁, s₄)) == TouchingLineSegment # CASE 2
    @test l₁ ∩ s₄ ≈ s₄ ∩ l₁ ≈ s₄(1)
    @test type(intersection(l₁, s₅)) == TouchingLineSegment # CASE 2
    @test l₁ ∩ s₅ ≈ s₅ ∩ l₁ ≈ s₅(0)
    @test type(intersection(l₁, s₆)) == OverlappingLineSegment # CASE 3
    @test l₁ ∩ s₆ ≈ s₆ ∩ l₁ ≈ s₆
  ...
  @testset "Triangles" begin
    ...
    @test type(intersection(s, t)) == IntersectingSegmentTriangle
    ...
    @test type(intersection(s, t)) == NoIntersection
    @test isnothing(s ∩ t)
    ...
    @test type(intersection(s, t)) == NoIntersection
    @test isnothing(s ∩ t)
    @test type(intersection(reverse_segment(s), t)) == NoIntersection
    ...
    t = Triangle(P3(0, 0, 0), P3(1, 0, 0), P3(0, 1, 0))
    s = Segment(P3(0.2, 0.2, 1.0), P3(0.2, 0.2, -1.0))
    @test type(intersection(t, s)) == IntersectingSegmentTriangle
    ...
    @test type(intersection(r, t)) == IntersectingRayTriangle
    ...
    r = Ray(P3(0.2, 0.2, 1.0), V3(0.0, 0.0, 1.0))
    @test type(intersection(r, t)) == NoIntersection
    @test isnothing(r ∩ t)
  ...
  @testset "Ngons" begin
    ...
    @test type(intersection(r, o)) == IntersectingRayTriangle
    @test r ∩ o ≈ P3(0.0, 0.0, 0.0)

    r = Ray(P3(-1.0, -1.0, -1.0), V3(-1.0, -1.0, -1.0))
    @test type(intersection(r, o)) == NoIntersection
    @test isnothing(r ∩ o)
  end
  ...
  @testset "LinePlanes" begin
    ...
    @test type(intersection(l, p)) == CrossingLinePlane
    @test l ∩ p == P3(0, 1, 1)
    ...
    @test type(intersection(l, p)) == OverlappingLinePlane
    @test l ∩ p == l
    ...
    l = Line(P3(0, 0, -1), P3(0, -2, -1))
    @test type(intersection(l, p)) == NoIntersection
    @test isnothing(l ∩ p)

    # plane as first argument
    p = Plane(P3(0, 0, 1), V3(1, 0, 0), V3(0, 1, 0))
    l = Line(P3(0, 0, 0), P3(0, 2, 2))
    @test type(intersection(p, l)) == CrossingLinePlane
  ...
  @testset "RayPlanes" begin
    ...
    @test type(intersection(r, p)) == CrossingRayPlane
    @test r ∩ p == P3(0, 1, 1)

    # intersecting ray and plane with λ ≈ 0
    r = Ray(P3(0, 0, 1), V3(0, 2, 1))
    @test type(intersection(r, p)) == TouchingRayPlane
    @test r ∩ p == P3(0, 0, 1)
    ...
    @test type(intersection(r, p)) == OverlappingRayPlane
    @test r ∩ p == r

    # ray below plane, non-intersecting
    r = Ray(P3(0, 0, 0), V3(0, -2, -2))
    @test type(intersection(r, p)) == NoIntersection
    @test isnothing(r ∩ p)
    ...
    p = Plane(P3(0, 0, 1), V3(1, 0, 0), V3(0, 1, 0))
    r = Ray(P3(0, 0, 0), V3(0, 2, 2))
    @test type(intersection(p, r)) == CrossingRayPlane
  ...
  @testset "SegmentPlanes" begin
    ...
    @test type(intersection(s, p)) == CrossingSegmentPlane
    @test s ∩ p == P3(0, 1, 1)

    # intersecting segment and plane with λ ≈ 0
    s = Segment(P3(0, 0, 1), P3(0, 2, 2))
    @test type(intersection(s, p)) == TouchingSegmentPlane
    @test s ∩ p == P3(0, 0, 1)
    ...
    @test type(intersection(s, p)) == OverlappingSegmentPlane
    @test s ∩ p == s

    # segment below plane, non-intersecting
    s = Segment(P3(0, 0, 0), P3(0, -2, -2))
    @test type(intersection(s, p)) == NoIntersection
    @test isnothing(s ∩ p)
    ...
    p = Plane(P3(0, 0, 1), V3(1, 0, 0), V3(0, 1, 0))
    s = Segment(P3(0, 0, 0), P3(0, 2, 2))
    @test type(intersection(p, s)) == CrossingSegmentPlane
  ...
  @testset "Boxes" begin
    ...
    @test type(intersection(b1, b2)) == OverlappingBoxes
    @test b1 ∩ b2 == Box(P2(0.5, 0.5), P2(1, 1))
    @test type(intersection(b1, b3)) == NoIntersection
    @test b1 ∩ b3 === nothing
    @test type(intersection(b1, b4)) == CornerTouchingBoxes
    @test b1 ∩ b4 == P2(1, 1)
    @test type(intersection(b1, b5)) == FaceTouchingBoxes
    ...
    @test type(intersection(r, b)) == CrossingRayBox
    ...
    @test type(intersection(r, b)) == NoIntersection

    r = Ray(P3(2.0, 0.0, 0.5), V3(-1.0, 1.0, 0.0))
    @test type(intersection(r, b)) == TouchingRayBox
