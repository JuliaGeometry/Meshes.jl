@testset "Predicates" begin
  ...
  @testset "isconvex" begin
    ...
    @test isconvex(t)
    # TODO: fix intersection(f, Segment, Segment) is required to run these tests
  ...
  @testset "issubset" begin
    ...
    # TODO: fix intersection(f, Segment, Segment) to run commented tests
    # tri = Triangle(point(0, 0), point(1, 0), point(1, 1))
    # box = Box(point(0, 0), point(1, 1))
    # ball = Ball(point(0, 0))
    # quad = Quadrangle(point(0, 0), point(1, 0), point(1, 1), point(0, 1))
  ...
  #   @test !intersects(point, point + vector(1, 1))
