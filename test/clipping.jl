@testitem "Clipping" setup = [Setup] begin
  for method in (SutherlandHodgmanClipping(), MartinezRuedaClipping())
    # triangle
    poly = Triangle(cart(6, 2), cart(3, 5), cart(0, 2))
    other = Quadrangle(cart(5, 0), cart(5, 4), cart(0, 4), cart(0, 0))
    clipped = clip(poly, other, method)
    ...
    other = Quadrangle(cart(5, 0), cart(5, 4), cart(0, 4), cart(0, 0))
    clipped = clip(poly, other, method)
    @test method == SutherlandHodgmanClipping() ? !issimple(clipped) : all(issimple.(rings(clipped)))
    ...
    if method == SutherlandHodgmanClipping()
      @test !issimple(clipped)
      @test all(
        vertices(crings[1]) .≈ [
          cart(1.5, 7.0),
