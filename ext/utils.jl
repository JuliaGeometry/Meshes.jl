...
function vizmany!(plot, objs, colorant)
  pointsize = plot[:pointsize]
  segmentsize = plot[:segmentsize]

  object = Makie.@lift reduce(concat, $objs)
  colors = Makie.@lift mayberepeat($colorant, $objs)

  viz!(plot, object, color=colors, pointsize=pointsize, segmentsize=segmentsize)
end
