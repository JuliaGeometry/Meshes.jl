...
function Boundary{P,Q}(topology) where {P,Q}
  ...
  return Boundary{P,Q,D,T}(topology)
end
...
function (∂::Boundary{D,0,D,T})(ind::Integer) where {D,T<:SimpleTopology}
  return collect(connec4elem(∂.topology, ind))
end
