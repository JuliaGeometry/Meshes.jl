...
function sample(::AbstractRNG, box::Box, method::RegularSampling)
  ...
  return ivec(or + (ind.I .- 1) .* sp for ind in CartesianIndices(sz))
end

function sample(
  ::AbstractRNG, bezier::BezierCurve{Dim,T}, method::RegularSampling
) where {Dim,T}
  ...
  return (bezier(t) for t in trange)
end

function sample(::AbstractRNG, sphere::Sphere{2,T}, method::RegularSampling) where {T}
  ...
  return ivec(c + r⃗(θ) for θ in θrange)
end
...
function sample(::AbstractRNG, sphere::Sphere{3,T}, method::RegularSampling) where {T}
  ...
  return ivec(c + r⃗(θ, φ) for θ in θrange, φ in φrange)
end
...
function sample(::AbstractRNG, ball::Ball{Dim,T}, method::RegularSampling) where {Dim,T}
  ...
  return ivec(scale(p, s) for p in points, s in srange)
end

function sample(
  ::AbstractRNG, cylsurf::CylinderSurface{T}, method::RegularSampling
) where {T}
  ...
  function point(φ, z)
    pᵦ, pₜ = cᵦ(φ), cₜ(φ)
    p = pᵦ + z * (pₜ - pᵦ)
    return Point((R' * coordinates(p)) + oₘ)
  end

  return ivec(point(φ, z) for φ in φrange, z in zrange)
end

function sample(::AbstractRNG, seg::Segment{Dim,T}, method::RegularSampling) where {Dim,T}
  sz = fitdims(method.sizes, paramdim(seg))
  trange = range(T(0), T(1); length=sz[1])
  return (seg(t) for t in trange)
end

function sample(
  ::AbstractRNG, quad::Quadrangle{Dim,T}, method::RegularSampling
) where {Dim,T}
  sz = fitdims(method.sizes, paramdim(quad))
  urange = range(T(0), T(1); length=sz[1])
  vrange = range(T(0), T(1); length=sz[2])
  return ivec(quad(u, v) for u in urange, v in vrange)
end

function sample(
  ::AbstractRNG, hex::Hexahedron{Dim,T}, method::RegularSampling
) where {Dim,T}
  sz = fitdims(method.sizes, paramdim(hex))
  urange = range(T(0), T(1); length=sz[1])
  vrange = range(T(0), T(1); length=sz[2])
  wrange = range(T(0), T(1); length=sz[3])
  return ivec(hex(u, v, w) for u in urange, v in vrange, w in wrange)
end

function sample(rng::AbstractRNG, grid::CartesianGrid, method::RegularSampling)
  return sample(rng, boundingbox(grid), method)
end
