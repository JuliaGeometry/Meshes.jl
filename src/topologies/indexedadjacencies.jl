"""
    IndAdjTopology{K, dim, K_} <: Topology

Indexed Adjacencies Topology (IA) described in
...
"""
struct IndAdjTopology{K, dim, K_} <: Topology
    ...
    """
    Note: K_ must be equal to K+1, but it's typically automatically computed.
    """
    function IndAdjTopology(
...
function IndAdjTopology(connections::AbstractVector{<:Connectivity{KSimplex{K, dim}, N}}) where {K, dim, N}
    ...
    IndAdjTopology(simplicies, neighboring_simplicies, R_star_relations)
end

paramdim(::IndAdjTopology{K}) where {K} = K

# ---------------------
# HIGH-LEVEL INTERFACE
# ---------------------

nvertices(t::IndAdjTopology) = length(t.R_star_relations)

# TODO: Implement this.
# function faces(t::IndAdjTopology, rank) end

element(t::IndAdjTopology, idx) = t.simplicies[idx]

nelements(t::IndAdjTopology) = length(t.simplicies)

# ------------
# CONVERSIONS
# ------------

function Base.convert(::Type{SimpleTopology}, t::IndAdjTopology)
