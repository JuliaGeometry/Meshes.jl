...
function Base.isapprox(p₁::Plane, p₂::Plane; atol=atol(lentype(p₁)), kwargs...)
  n₁ = withunit.(normal(p₁), unit(lentype(p₁)))
  n₂ = withunit.(normal(p₂), unit(lentype(p₂)))
  isapproxzero(udot(p₁(0, 0) - p₂(0, 0), n₂); atol, kwargs...) &&
    isapproxzero(udot(p₂(0, 0) - p₁(0, 0), n₁); atol, kwargs...) &&
    isapproxzero(norm(ucross(n₁, n₂)); atol, kwargs...)
end
