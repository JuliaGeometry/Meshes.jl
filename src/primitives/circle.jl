...
See also [`Disk`](@ref).
"""
struct Circle{P<:Plane,ℒ<:Len} <: Primitive{3}
  plane::P
  radius::ℒ
  Circle{P,ℒ}(plane, radius) where {P<:Plane,ℒ<:Len} = new(plane, radius)
end

Circle(plane::P, radius::ℒ) where {P<:Plane,ℒ<:Len} = Circle{P,float(ℒ)}(plane, radius)
...
function Circle(p1::Point{3}, p2::Point{3}, p3::Point{3})
  v12 = p2 - p1
  v13 = p3 - p1
  m12 = fromorigin(p1 + v12 / 2)
  m13 = fromorigin(p1 + v13 / 2)
...
lentype(::Type{<:Circle{P}}) where {P} = lentype(P)
...
Random.rand(rng::Random.AbstractRNG, ::Random.SamplerType{Circle}) = Circle(rand(rng, Plane), rand(rng, Met{Float64}))
