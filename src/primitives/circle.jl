...
See also [`Disk`](@ref).
"""
struct Circle{C<:CRS,M<:AbstractManifold,P<:Plane{C,M},ℒ<:Len} <: Primitive{C,M}
  plane::P
  radius::ℒ
  Circle(plane::P, radius::ℒ) where {C<:CRS,M<:AbstractManifold,P<:Plane{C},ℒ<:Len} = new{C,M,P,float(ℒ)}(plane, radius)
end
