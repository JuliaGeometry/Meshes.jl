...
"""
struct Cylinder{M<:AbstractManifold,C<:CRS,P<:Plane{M,C},ℒ<:Len} <: Primitive{M,C}
  bot::P
  top::P
  radius::ℒ
  Cylinder(bot::P, top::P, radius::ℒ) where {M<:AbstractManifold,C<:CRS,P<:Plane{M,C},ℒ<:Len} = new{M,C,P,float(ℒ)}(bot, top, radius)
end
