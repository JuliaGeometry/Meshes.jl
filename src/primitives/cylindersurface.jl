...
"""
struct CylinderSurface{C<:CRS,M<:AbstractManifold,P<:Plane{C},ℒ<:Len} <: Primitive{C,M}
  bot::P
  top::P
  radius::ℒ
  CylinderSurface(bot::P, top::P, radius::ℒ) where {C<:CRS,M<:AbstractManifold,P<:Plane{C},ℒ<:Len} = new{C,M,P,float(ℒ)}(bot, top, radius)
end
