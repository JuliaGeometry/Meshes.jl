...
See also [`Ball`](@ref).
"""
struct Sphere{C<:CRS,M<:AbstractManifold,ℒ<:Len} <: Primitive{C,M}
  center::Point{C,M}
  radius::ℒ
  Sphere(center::Point{C,M}, radius::ℒ) where {C<:CRS,M<:AbstractManifold,ℒ<:Len} = new{C,M,float(ℒ)}(center, radius)
end
