...
# Fallback: inverse if possible, otherwise error
function _reversetransform(t::Transform)
	isrevertible(t) ? inverse(t) : throw(ArgumentError("Irreversible transform: $(typeof(t))"))
end

# Treat specific irreversible transforms as Identity (no-op)
_reversetransform(::Morphological) = Identity()
_reversetransform(::Repair) = Identity()
_reversetransform(::Bridge) = Identity()

function _reversetransform(st::SequentialTransform)
	# Recursively reverse steps, replacing exceptions with Identity
	newsteps = map(_reversetransform, st.transforms)
	reduce(→, reverse(newsteps))
end

function indices(grid::TransformedGrid, geometry::Geometry)
	# construct reverse transform from revertible steps
	revtrans = _reversetransform(transform(grid))
