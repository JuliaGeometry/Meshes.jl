...
```
"""
struct CartesianGrid{Dim,P<:Point{Dim},T} <: Grid{Dim}
  origin::P
...
function CartesianGrid(start::Point{Dim}, finish::Point{Dim}, spacing::NTuple{Dim,T}) where {Dim,T}
